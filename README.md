...
```bash
serverless-mlops$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
serverless-mlops$ sam local start-api
serverless-mlops$ curl http://localhost:3000/classify_digit
```
...
```bash
serverless-mlops$ sam logs -n InferenceFunction --stack-name serverless-mlops --tail
```
...
```bash
aws cloudformation delete-stack --stack-name serverless-mlops
```
